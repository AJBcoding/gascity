...
| Medium owner | `scripts` package `scripts_test` | TestShippedCloseGraduationReleaseCheck: subprocess | gas-ftv4 | shipped-close graduation gate proof is a checked Medium owner; the one bash invocation of scripts/check-shipped-close-graduation.sh is confined to TestShippedCloseGraduationReleaseCheck, which runs the real release gate to prove it refuses a v1.6.0 removal release while stale beads.shipped_close_warn_only migration artifacts remain; the call retires when that compatibility flag graduates | P0.4b | 2026-10-01 |
