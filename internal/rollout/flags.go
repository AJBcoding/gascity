...
type Flags struct {
	...
	shippedCloseWarnOnly   resolved[bool]
...
func (f Flags) OriginOf(key string) Origin {
	...
	case keyBeadsShippedCloseWarnOnly:
		return f.shippedCloseWarnOnly.origin
...
func (f Flags) ValueOf(key string) string {
	...
	case keyBeadsShippedCloseWarnOnly:
		return strconv.FormatBool(f.shippedCloseWarnOnly.value)
	case keyDaemonFormulaV2:
