...
func TestPlacementFor(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "wisp id carrying a non-gc city prefix still gets a themed tab",
		...
		{
			name:     "wisp id with a digit-bearing city prefix still gets a themed tab",
			sessName: "gastown__polecat-n990-wisp-abc123",
			env:      map[string]string{"GC_RIG": "n990", "GC_ALIAS": "n990/gastown.slit"},
			wantWS:   "n990",
			wantTab:  "polecat-slit",
		},
		{
			name:     "wisp id with no role prefix becomes the bare alias",
		...
		{
			name:     "deferred pool spawn: blank alias and GC_AGENT holding the session name keeps the wisp tab",
			sessName: "gastown__polecat-az-wisp-0pd1u",
			env:      map[string]string{"GC_RIG": "python419", "GC_ALIAS": "", "GC_AGENT": "gastown__polecat-az-wisp-0pd1u"},
			wantWS:   "python419",
			wantTab:  "polecat-az-wisp-0pd1u",
		},
		{
			name:     "non-gc alias that is itself the wisp identity is ignored",
			sessName: "gastown__polecat-az-wisp-abc123",
			env:      map[string]string{"GC_RIG": "gascity", "GC_AGENT": "gastown.polecat-az-wisp-abc123"},
			wantWS:   "gascity",
			wantTab:  "polecat-az-wisp-abc123",
		},
		{
