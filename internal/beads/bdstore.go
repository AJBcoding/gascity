...
type BdStore struct {
	...
	// Ready-projection capability (the `bd sql` version gate and its failure
	// latch) is NOT held here: it is keyed by store directory in
	// readyProjectionCapabilities so it survives the short-lived stores the
	// control-dispatcher path rebuilds every drain sweep (gas-x5k4).
	//
	// Upstream reintroduced per-store readyProjectionMu/Checked/Enabled fields
	// on this struct in the 391-commit merge. They are deliberately NOT taken:
	// bdstore_ready_projection.go supersedes them, nothing in the merged tree
	// reads them, and holding the latch per-store is the exact shape gas-x5k4
	// fixed.
