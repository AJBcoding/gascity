...
//
// THIS ASSEMBLY is one of the assemblies that clause anticipates: it serves
// beads over MySQL (`gc beads city use-mysql`, 13 live scopes) and carries the
// postgres shape alongside it, so both names are registered here and the pin
// below is four names rather than OSS's two. The refusal probe therefore uses a
// name no build registers. Keep this list in step with compiledBackendNames().
