...
func TestLoadMetadataStateRejectionOrderIsPinned(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Adapted from upstream's E2-refuses-complete-keys case, which
			// named "postgres" as the unsupported backend. This lane registers
			// postgres and mysql (upstream #5144 is dolt-only), so the
			// exemplar moves to a backend neither side compiles. The point is
			// unchanged and is the reason the keys are still spelled in a real
			// backend's shape: a complete key set does not rescue a backend
			// name the registry does not know.
			name:     "E2 refuses a backend whose own keys are complete",
			metadata: `{"backend":"cockroach","postgres_host":"h","postgres_port":"5432","postgres_user":"bd","postgres_database":"beads"}`,
			want:     `unsupported backend "cockroach"`,
			loses:    "invalid metadata.json:",
		},
	}
	for _, tc := range cases {
...
// lives in config.yaml), and it removes nothing at all for a backend gc does
// not write — a key gc does not write is not gc's to delete.
//
// Adapted from upstream (#5144), which asserted "nothing at all for every
// other backend" because dolt was the only compiled backend there, making
// every other name unwritten by definition. This lane compiles postgres and
// mysql too, so gc does write their keys and a scope on one registered
// backend must have the others' keys scrubbed. That is the same rule reaching
// further, not a different rule: the set tracks the registry, so the
// assertion is written against compiledBackendNames rather than a fixed list
// and stays honest as backends are added or retired.
func TestCrossBackendKeysToScrubOnlyRemovesKeysGCWrites(t *testing.T) {
	scrub := crossBackendKeysToScrub("doltlite")
	if len(scrub) == 0 || scrub[0] != "dolt_mode" {
		t.Fatalf("crossBackendKeysToScrub(doltlite) = %v, want it to lead with dolt_mode", scrub)
	}
	// The config key is never the metadata key, for any backend.
	for _, name := range compiledBackendNames() {
		backend := string(name)
		for _, key := range crossBackendKeysToScrub(backend) {
			if key == "dolt.mode" {
				t.Fatalf("crossBackendKeysToScrub(%q) = %v, want it to never scrub the config key dolt.mode", backend, key)
			}
		}
	}
