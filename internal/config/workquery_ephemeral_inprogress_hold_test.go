...
// behavior rather than the script's spelling. That matters more here than on
// the bd-list tier: this lane memoizes the ephemeral scan into a shell var
// (ephemeralScanMemoShell), so the probe's jq reads `printf "%s" "$memo"`
// rather than a pipe straight off `bd query`, and only executing the script
// proves the guard survives that indirection.
//
...
// consumes the memoized `bd query` stdout, so garbage yields an empty candidate
// and the probe falls through. It must never serve raw non-JSON stdout, and the
