...
func (g *doltLeakGuardedTestingM) runWith(
	...
	if initialErr == nil {
		...
		// Upstream passes its grace bounds down to waitForFinalScanToClear; this
		// lane settles instead, so the same two knobs drive settleDoltLeaks —
		// otherwise runWith would accept tuning parameters it silently ignored.
		// Zero means "caller did not tune", so fall back to the lane defaults.
		settleWindow, settleInterval := graceMaxElapsedTime, graceInitialInterval
		if settleWindow <= 0 {
			settleWindow = doltLeakSettleWindow()
		}
		if settleInterval <= 0 {
			settleInterval = doltLeakSettleInterval
		}
		leaked, finalErr := settleDoltLeaks(scan, initial, settleWindow, settleInterval)
		if finalErr != nil {
...
func reapDoltLeakProcessesWithKiller(leaked []DoltProcInfo, killFn func(int, syscall.Signal) error) []error {
	...
	// Upstream's reaper, not the lane's former reapLeakPIDsWithKiller: it waits
	// for the signaled pid to actually leave the process table rather than
	// returning once the signal call succeeds (ga-62mu45). Strictly safer for
	// callers racing a TempDir cleanup behind the reap, so the lane's
	// fire-and-forget form is dropped rather than kept alongside it.
...
// reapLeakPIDsWithKiller is the backend-agnostic spelling of the reaper above,
// kept because this lane's tmux leak guard (reapTmuxLeakProcesses) reaps pids
// that are not Dolt servers and reads better without the Dolt-specific name.
// It is a pure alias — same wait-for-actual-exit behavior (ga-62mu45) — so
// there is one reaper implementation, not two.
