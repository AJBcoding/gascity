...
func TestEvaluateWorkRecordCloseGate(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			block := evaluateWorkRecordCloseGate(tc.args, newStore(), t.TempDir(), nil, tc.enforce, &stderr)
...
func TestEvaluateWorkRecordCloseGateNonOriginRemote(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-upstream", commit, "main"), gateStoreWith("wr-upstream", repoDir), repoDir, nil, true, &stderr); block {
	...
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-upstream", commit, "gc-gastown.nux-1a2b3c4d5e6f"), gateStoreWith("wr-upstream", repoDir), repoDir, nil, true, &staleStderr); block {
...
func TestEvaluateWorkRecordCloseGateUnpushedOnBranchSingleViolation(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-onbranch", commit, workBranch), gateStoreWith("wr-onbranch", repoDir), repoDir, nil, true, &stderr); !block {
...
func TestEvaluateWorkRecordCloseGateRemotelessStaleStamp(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-remoteless", commit, "gc-gastown.nux-1a2b3c4d5e6f"), gateStoreWith("wr-remoteless", repoDir), repoDir, nil, true, &stderr); block {
...
func TestEvaluateWorkRecordCloseGateMissingStampDelivered(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(args, gateStoreWith("wr-nostamp", repoDir), repoDir, nil, true, &stderr); block {
	...
	if block := evaluateWorkRecordCloseGate(undeliveredArgs, gateStoreWith("wr-nostamp", repoDir), repoDir, nil, true, &undeliveredStderr); !block {
...
func TestEvaluateWorkRecordCloseGateAdvisoryPrefersDefaultBranch(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-prefer", commit, "gc-gastown.nux-1a2b3c4d5e6f"), gateStoreWith("wr-prefer", repoDir), repoDir, nil, true, &stderr); block {
...
func TestEvaluateWorkRecordCloseGateAtomicShippedUpdate(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(args, store, repoDir, nil, true, &stderr); block {
...
func TestEvaluateWorkRecordCloseGateUnpushedBranch(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(args, newStore(), repoDir, nil, true, &stderr); !block {
	...
	if block := evaluateWorkRecordCloseGate(args, newStore(), repoDir, nil, true, &pushedStderr); block {
...
func TestEvaluateWorkRecordCloseGateStaleClaimStamp(t *testing.T) {
	...
	if block := evaluateWorkRecordCloseGate(unpushedArgs, newStore(), repoDir, nil, true, &unpushedStderr); !block {
...
func TestEvaluateWorkRecordCloseGateSelfRemote(t *testing.T) {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(closeArgs(workBranch), newStore(), repoDir, nil, true, &stderr); !block {
	...
	var staleStderr strings.Builder
	if block := evaluateWorkRecordCloseGate(closeArgs("gc-gastown.nux-1a2b3c4d5e6f"), newStore(), repoDir, nil, true, &staleStderr); !block {
	...
	runGit(t, repoDir, "push", "origin", workBranch)
	var pushedStderr strings.Builder
	if block := evaluateWorkRecordCloseGate(closeArgs(workBranch), newStore(), repoDir, nil, true, &pushedStderr); block {
...
func TestEvaluateWorkRecordCloseGateCommitInRigRepo(t *testing.T) {
	...
	if err := exec.Command("git", "-C", cityRepo, "cat-file", "-e", commit+"^{commit}").Run(); err == nil {
	...
	var stderr strings.Builder
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-cross-repo", commit, workBranch), gateStoreWith("wr-cross-repo", cityRepo), cityRepo, []string{rigRepo}, true, &stderr); block {
	...
	var unpushedStderr strings.Builder
	if block := evaluateWorkRecordCloseGate(gateShippedCloseArgs("wr-cross-repo", unpushed, unpushedBranch), gateStoreWith("wr-cross-repo", cityRepo), cityRepo, []string{rigRepo}, true, &unpushedStderr); !block {
