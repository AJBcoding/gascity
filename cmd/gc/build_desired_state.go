...
func defaultScaleCheckCountsAndDemand(cfg *config.City, targets []defaultScaleCheckTarget, caches ...*readyDemandCache) (map[string]int, map[string]scaleCheckDemand, map[string]bool, []error) {
	...
	for key, group := range groups {
		...
		for _, b := range ready {
			...
			// (Subsumes the lane's gas-qckc hold guard: the predicate reads
			// PoolDemandServeRulesForQuery, whose ExcludeLabels is
			// beadmeta.DispatchHoldLabels — the demand tier and the serve tier
			// now share one descriptor.)
...
func poolTriggerWorkDir(bp *agentBuildParams, cfgAgent *config.Agent, qualifiedName string, request SessionRequest) string {
	...
	// Path-only: this computes a metadata value for the trigger binding patch.
	// Materializing here would mint a husk dir under the template identity
	// (gas-m7h).
...
func prepareTemplateResolution(bp *agentBuildParams, cfgAgent *config.Agent, qualifiedName string, stderr io.Writer) {
	...
	// Path-only: the stagers below (hooks.Install, the overlay stager) create
	// the directories they write into, so resolving must not pre-create one.
	// Otherwise an agent with nothing to stage still gets an empty dir minted
	// every tick — the husk shape from gas-m7h.
	workDir, err := resolveConfiguredWorkDirPath(bp.cityPath, bp.cityName, qualifiedName, cfgAgent, bp.rigs)
	if err != nil {
		if stderr != nil {
