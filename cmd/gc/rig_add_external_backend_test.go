...
// readScopeMetadataMap returns the raw on-disk metadata keys for a scope so
// tests can assert on cross-backend key absence as well as the parsed shape.
func readScopeMetadataMap(t *testing.T, scopeRoot string) map[string]any {
	...
	if err != nil {
		t.Fatalf("read scope metadata: %v", err)
	}
	var raw map[string]any
	if err := json.Unmarshal(data, &raw); err != nil {
		t.Fatalf("parse scope metadata: %v", err)
	}
	return raw
}
...
func TestScopeSkipsManagedDoltForInitForFreshRigOnMySQLCity(t *testing.T) {
	t.Setenv("GC_BEADS", "bd")
	cityPath := t.TempDir()
	writeExternalBackendCity(t, cityPath)
	rigPath := t.TempDir()
