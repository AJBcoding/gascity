...
Say which target you mean in the bead/PR. Two verification habits, from
az-4nxd: confirm what is deployed against the binary
(`go version -m ~/.local/gc-bin/gc`, symbol greps), not against whatever
tree you happen to be reading; and never judge publication with a blanket
...
- When a bead needs to pause on a specific actor or condition, only `hold:mayor` and `hold:external` are canonical (set via `bd set-state <id> hold=mayor|external --reason "..."`) — never invent a new ad hoc hold/blocked label. Release a hold the same way, with `bd set-state <id> hold=none --reason "..."` — never `bd update --remove-label`, which files no audit event. See `engdocs/contributors/hold-label-conventions.md`.
